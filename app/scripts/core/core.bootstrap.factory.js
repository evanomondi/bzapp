(function () {
    ...
    function bootstrap(
            ...
            if (isComplete) {
                ...
                return setLanguage().then(function () {
                    ...
                    if ($ionicConfig.backButton.text()) {
                        $translate('common.back').then(function (value) {
                    ...
                    return $translate('error.dataError').then(function (value) {
                        ...
                        .then(function () {
                            return userManager.loadUser().catch(function () {});
                        })
                        .then(function () {
                            return loadShopData();
                        })
                        .then(function () {
                            addAnalyticsTrackers();
                        })
                        .then(function () {
                            if (window.cordova) {
        ...
        function addAnalyticsTrackers() {
            var trackerConfig = AppContext['TRACKERS'] || {};

            Object.keys(trackerConfig).forEach(function (trackerName) {
                var tracker;

                try {
                    tracker = $injector.get(trackerName + 'Tracker');
                } catch (err) {
                    tracker = null;
                }
