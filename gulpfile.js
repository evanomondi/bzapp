...
var installPlugin = require('./tasks/plugin.install.js').install;
...
var plugin = require('./tasks/plugin.use.js')(appSettings, targetDir);
...
function buildScripts() {
    ...
    var scriptStream = gulp
        .src(['**/*.js', '!plugins/!(' + plugin.getName() + '|plugin.module.js){,/**}', '!plugins/' + plugin.getName() + '/install/**'], {
            cwd: 'app/scripts'
        })
...
function jsHint() {
    return gulp
        .src(['**/*.js', '!categories/categories.constants.js', '!plugins/**/install/**'], {
            cwd: 'app/scripts'
        })
        .pipe(plugins.jshint())
...
function serve(done) {
    ...
    if (!build) {        
        var pluginSettings = plugin.getSettings(),
            apiHost = pluginSettings['API_HOST'];
        if (apiHost) {
            proxyOptions = url.parse(apiHost);            
            proxyOptions.dropCookiePath = true;
            server.use('/' + plugin.getName() + '/api', proxy(proxyOptions));
        }        
    }
...
gulp.task('ionic:emulate', plugins.shell.task([
    'ionic cordova emulate ' + emulate + ' --livereload --consolelogs'
]));
...
function watchers(done) {
    ...
    gulp.watch('config/settings.*yml', gulp.series(loadSettings, plugin.clean, plugin.use, styles, index));
    gulp.watch(targetDir + '/**/*')
        .on('change', debounce(plugins.livereload.changed, 2000))
        .on('error', errorHandler);
    done();
}
...
gulp.task('install:plugin', installPlugin);